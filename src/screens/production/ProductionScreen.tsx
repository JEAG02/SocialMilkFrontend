...
export default function ProductionScreen({ navigation }: any) {
  ...
  const loadProductions = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Error cargando producción");
      }
  ...
  const loadSummary = async (animalId: string, period: string) => {
    try {
      const token = await AsyncStorage.getItem("token");
  ...
  const handleCreate = async () => {
    try {
      if (!liters || !shift.trim() || !selectedAnimal) {
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.formCard}>
              <Text style={styles.sectionTitle}>Nueva Producción</Text>
              ...
              <Text style={styles.label}>Selecciona animal</Text>
                ...
                {animals.map((animal: any) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      loadSummary(animal.animalId, summaryPeriod);
                    }}
                  >
                    <Ionicons
                      name="paw"
                      size={24}
                      color={
                        selectedAnimal === animal.animalId ? "#fff" : "#3b82f6"
                      }
                    />
            ...
            {!!selectedAnimal && (
              <View style={styles.summaryCard}>
                <Text style={styles.summaryTitle}>Resumen producción</Text>
                  ...
                  {["daily", "weekly", "monthly"].map((period) => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        loadSummary(selectedAnimal, period);
                      }}
                    >
                      <Text
                        style={[
            ...
            <View style={styles.historyContainer}>
              ...
              {productions.map((item) => {
                const animal = animals.find(
                  (a: any) => a.animalId === item.animalId,
                );
                ...
                return (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.productionLiters}>
                        {item.milkVolumeLiters} L
                      </Text>

                      <Text style={styles.productionAnimal}>
                        🐄 {animal?.animalName || "Animal desconocido"}
                      </Text>

                      <Text style={styles.productionInfo}>
                        Turno {item.shift}
                      </Text>
